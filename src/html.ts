...
<div class="plan-name">${escapeHtml(planDisplayName(plan.plan))}<span class="plan-sub">${fmtInt(plan.includedCredits)} credits · ${fmtUsd(plan.includedCredits / 100)}/month</span></div>
<div class="track"><div class="usage" style="width:${Math.min(100, pct).toFixed(1)}%"></div></div>
<div class="used ${cls}"><strong>${fmt(pct)}%</strong></div>
</div>`;
      })
      .join("");
...
function renderSources(summary: Summary): string {
  ...
  return sources
    .map(([source, credits], i) => {
      ...
      return `<div class="source-row"><span class="source-name">${DISPLAY_NAMES[source] ?? source}</span><div class="source-track"><div class="source-fill" style="width:${pct.toFixed(1)}%;background:${color}"></div></div><div class="source-val"><strong>${fmt(pct)}%</strong><span>${fmtCredits(credits)} cr</span></div></div>`;
    })
    .join("");
}
...
function renderWarnings(summary: Summary): string {
  const warnings = summary.sources.flatMap((s) =>
...
export function renderHtml(summary: Summary): string {
  ...
  const warnings = renderWarnings(summary);
  const autoModelNote = renderAutoModelNote(summary);
...
${warnings}${autoModelNote}<section class="card"><div class="section-head"><h2 class="section-title">Usage trend</h2><div class="head-meta">${fmtCredits(summary.totals.credits)} credits in range · ${comparison.label}: ${fmtCredits(comparison.credits)}</div></div><div class="chart">${renderTrend(trend)}</div></section>
<div class="middle"><section class="card plan-card"><div class="section-head"><div><h2 class="section-title">Included credit comparison (${escapeHtml(comparison.label.toLowerCase())})</h2><div class="plan-subtitle"><strong>${fmtUsd(comparison.credits / 100)}</strong> · ${fmtCredits(comparison.credits)} credits</div></div></div><div class="plan-groups">${renderPlanGroups(summary, comparison.credits)}</div></section>
<section class="card source-card"><h2 class="section-title">Usage by source (total)</h2><div class="source-bars">${sources}</div></section></div>
<section class="card"><div class="section-head"><h2 class="section-title">Model usage and cost (total)</h2></div><table><thead><tr><th>Model</th><th class="right">Calls</th><th class="right">Input</th><th class="right">Cache read</th><th class="right">Output</th><th class="right">Credits</th><th class="right">Share</th></tr></thead><tbody>${renderModelRows(summary)}</tbody></table></section>
